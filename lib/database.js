        ...
        return {
            ...
            update: function(table, objectToSave, cb){
                ...
                if(!objectToSave._id) return cb(err); // returns error if object has no id 
                    ...
                    this.get(filePath, objectToSave._id, (err, result) => {
                        result = objectToSave; // old result becomes new obj with updates
                        fs.readFile(filePath, (err, data) => {
                            ...
                            fs.writeFile(filePath, JSON.stringify(updatedData), (err) => {
                                if (err) cb(err);
                                else cb(null, result); // return updated saved object
                            })
                        })
                    })
                }
